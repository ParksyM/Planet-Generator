...
public class OctahedronSphereCreator {
    ...
    public static Mesh Create(int subdivisions, float radius)
    {
        ...
        {
            subdivisions = 0;
            Debug.LogWarning("Octahedron Sphere subdivisions set to 0 as minimum.");
        }
        else if (subdivisions > MAX_SUBDIVISIONS)
        {
            subdivisions = MAX_SUBDIVISIONS;
            Debug.LogWarning("Octahedron Sphere subdivisions set to" + MAX_SUBDIVISIONS + " as maximum.");
        }
        ...
        Mesh mesh = new Mesh();
        mesh.name = "Octahedron Sphere Level " + subdivisions;
    ...
    private static void Normalise(Vector3[] vertices, Vector3[] normals)
    {
        for (int i = 0; i < vertices.Length; i++)
        {
